...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var m1 = new Matrix(50, 50, true);
            var m2 = new Matrix(50, 50, true);

            var multiplierRegular = new MatricesMultiplier();
            var multiplierParallel = new MatricesMultiplierParallel();

            Stopwatch stopWatchRegular = new Stopwatch();
            stopWatchRegular.Start();
            multiplierRegular.Multiply(m1, m2);
            stopWatchRegular.Stop();
            var regularMillisecs = stopWatchRegular.ElapsedMilliseconds;

            Stopwatch stopWatchParallel = new Stopwatch();
            stopWatchParallel.Start();
            multiplierParallel.Multiply(m1, m2);
            stopWatchParallel.Stop();
            var parallelMillisecs = stopWatchParallel.ElapsedMilliseconds;

            Assert.IsTrue(parallelMillisecs < regularMillisecs);
        }
